...
function NavItem({ href, icon: Icon, label, active = false }: { href: string, icon: any, label: string, active?: boolean }) {
    return (
        <Link href={href} className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors group cursor-pointer ${active ? 'bg-blue-500/10 text-accent' : 'text-text-secondary hover:bg-surface hover:text-text-primary'}`}>
            <Icon size={18} className={active ? 'text-accent' : 'text-text-tertiary group-hover:text-text-secondary'} />
                ...
                <div className="flex h-full">
                    ...
                    <aside className="w-[240px] flex-shrink-0 bg-panel border-r border-border flex flex-col z-20">
                        ...
                        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                            ...
                            {/* In a real app, 'active' would be determined by pathname hooks */}
